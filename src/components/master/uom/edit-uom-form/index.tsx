...
export default function EditUomForm(props: TProps) {
  const { values, isVisible, onCancel } = props;
  const [isSubmitting, setIsSubmitting] = useState(false);

  function handleEditUom(values: unknown) {
    try {
  ...
  return (
    <Modal
      className={styles.editUomModal}
      visible={isVisible}
      footer={undefined}
      closeIcon={undefined}
      onCancel={onCancel}
      header={<Title heading={4}>Edit Uom</Title>}
    >
      <Form className={styles.uomForm} onSubmit={handleEditUom}>
        <Form.Input
          rules={[{ required: true, message: 'satuan harus diisi' }]}
          field="uomName"
          label="Satuan"
          placeholder="Masukkan satuan"
          initValue={values?.uomName}
        />
        <Form.Input
          field="description"
          label="Deskripsi"
